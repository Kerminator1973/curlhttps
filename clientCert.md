...
Клиентский сертификат может быть встроен в приложение, либо размещён в локальном хранилище сертификатов (например, используя утилиту mmc). При клиентских сертификат есть в локальном хранилище, то при попытке подключиться к серверу, который требует пользовательский сертификат, браузер (Edge, Chrome) выведен на экран список установленных клиентских сертификатов и предложит выбрать тот из них, который следует передать серверу.
...
- `Out-File "c:\computer_filtered.txt"` - указывает имя файла, в который следует сохранить сформированные дданные
