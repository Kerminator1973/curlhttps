...
**Update 2024 года**: в число рассматриваемых библиотек, кроме curllib и boost.beast, также имеет смысл рассматривать [POCO C++ Libraries](https://pocoproject.org/). Эта библиотека является кросс-платформенной, интернет-центричной. Кроме сетевого взаимодействия, библиотека содержит реализацию адаптеров к популярным СУБД, в частности: ODBC, MySQL, SQLite.
