...
В соответствии с официальной инструкцией, после сборки следует выполнить команду `nmake test` для проверки корректности сборки и `nmake install` для развертывания компонентов openSSL в системе.

Поскольку процесс сборки библиотек является длительным, целесообразность разработки скрипта, который собирает библиотеки "одним кликом" кажется микро-оптимизацией. Более разумным может быть сборка всех скриптов вручную, в рамках отдельного микро-проекта.
